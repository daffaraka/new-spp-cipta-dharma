        ...
        <div class="card-body">
            <div class="mb-3">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <p class="mb-1"><strong>Total Pembayaran:</strong> {{ number_format($pembayaran->biaya->nominal) }}</p>
                        <p class="mb-1">
                            <strong>Status:</strong>
                            <span class="badge rounded-pill text-bg-danger">{{ $pembayaran->status }}</span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <tr>
                            <td>{{ \Carbon\Carbon::parse($pembayaran->tanggal_lunas ?? \Carbon\Carbon::now())->format('d-m-Y') }}</td>
                            <td>{{ number_format($pembayaran->biaya->nominal) }}</td>
                            <td>
                                @if ($pembayaran->bukti_pelunasan)
                                    <a href="{{ asset('bukti-pelunasan/' . $pembayaran->bukti_pelunasan) }}" class="btn btn-sm btn-primary" target="_blank">View</a>
                                @else
                                    -
                                @endif
                            </td>
                            <td><span class="badge rounded-pill text-bg-danger">{{ $pembayaran->status }}</span></td>
                            <td>-</td>
                        </tr>
                    </tbody>
                </table>
