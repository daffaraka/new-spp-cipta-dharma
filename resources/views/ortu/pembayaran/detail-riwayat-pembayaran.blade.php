    ...
    <div class="card">
        <div class="card-body py-4">
            <div class="mb-4">
                ...
                <p class="mb-2"><strong>Tanggal:</strong> {{ \Carbon\Carbon::parse($pembayaran->tanggal_terbit ?? \Carbon\Carbon::now())->format('d-m-Y') }}</p>
                <p class="mb-2"><strong>Nominal:</strong> {{ number_format($pembayaran->biaya->nominal ?? 500000) }}</p>
                <p class="mb-2">
                    <strong>Status:</strong>
                    @if($pembayaran->status == 'Lunas')
                        <span class="badge rounded-pill text-bg-success px-3">Sukses</span>
                    @elseif($pembayaran->status == 'Sedang Diverifikasi')
                        <span class="badge rounded-pill text-bg-warning px-3">Diproses</span>
                    @else
                        <span class="badge rounded-pill text-bg-danger px-3">Belum Lunas</span>
                    @endif
                </p>
            </div>
            ...
            <div class="d-flex justify-content-end gap-2">
                <a href="{{route('tagihan.lihatKuitansi', $pembayaran->id)}}" class="btn btn-lg btn-primary">Lihat Kuitansi</a>
                {{-- <a href="" class="btn btn-secondary">Unduh Kuitansi</a> --}}
            </div>
        </div>
    </div>
@endsection
